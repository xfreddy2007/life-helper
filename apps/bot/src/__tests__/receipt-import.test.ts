...
describe('formatReceiptPreview', () => {
  ...
  it('shows receiptName → resolvedName when they differ', () => {
    const item = makeItem({ receiptName: '特選米', resolvedName: '白米' });
    const text = formatReceiptPreview([item]);
    expect(text).toContain('特選米 → 白米');
  });

  it('shows only resolvedName when receipt and resolved names are the same', () => {
    const item = makeItem({ receiptName: '橄欖油', resolvedName: '橄欖油' });
    const text = formatReceiptPreview([item]);
  ...
  it('lists all items', () => {
    const items = [
      makeItem({ resolvedName: '白米', receiptName: '白米' }),
      makeItem({ resolvedName: '橄欖油', receiptName: '橄欖油', unit: '瓶' }),
      makeItem({ resolvedName: '鹽', receiptName: '鹽', unit: '包' }),
    ];
  ...
  it('handles single item correctly', () => {
    const text = formatReceiptPreview([
      makeItem({ resolvedName: '糖', receiptName: '糖', unit: 'kg', quantity: 1 }),
    ]);
